...
export default function Stamper(props) {
    ...
    if(props.hasBiscuitToStamp) {
        doughElement=(
            <RawBiscuitSVG data-testid='raw-biscuit' className={'raw-biscuit ' + animationPausedClass}/>
        );
    }
    return (
        <div className='stamper left'>
                <StamperSVG />
                <StamperPusherSVG data-testid='stamper-pusher' className={stamperPusherClassName} />
            {doughElement}
        </div>
    );
}
